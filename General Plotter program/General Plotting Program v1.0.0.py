...
class GeneralPlotter(tk.Tk):
    ...
    def apply_handoff_from_json(self, payload_path: str | None) -> None:
        """Load and apply GL-260 handoff payload data when provided.

        Purpose:
            Ingest optional launcher payload files generated by the GL-260 app.
        Why:
            The General Plotter runs as a separate process and needs an explicit
            transfer contract for dataset + selected column context.
        Inputs:
            payload_path: Optional path to a JSON payload containing dataset and
        ...
        if not os.path.exists(payload_path):
            messagebox.showerror(
                "General Plotter",
                f"Handoff payload was not found:\n{payload_path}",
            )
            return
        ...
        try:
            ...
            messagebox.showerror(
                "General Plotter",
                f"Failed to read handoff payload:\n{exc}",
            )
            return

        dataset_path = str(payload.get("dataset_csv_path") or "").strip()
        if not dataset_path or not os.path.exists(dataset_path):
            messagebox.showerror(
                "General Plotter",
                "Handoff payload did not include a valid dataset CSV path.",
            )
            return

        try:
            handoff_df = pd.read_csv(dataset_path)
        except Exception as exc:
            messagebox.showerror(
                "General Plotter",
                f"Failed to load handoff dataset CSV:\n{exc}",
            )
            return
        ...
        try:
            self.cb_sheet.configure(values=[])
        except Exception:
            pass
        self._post_load_dataframe()
        ...
        if not isinstance(selected_columns, dict):
            selected_columns = {}
        x_col = str(selected_columns.get("x") or "").strip()
        y1_col = str(selected_columns.get("y1") or "").strip()

        if x_col and x_col in self.columns:
            self.cb_x.set(x_col)
        self.lb_y.selection_clear(0, tk.END)
        if y1_col and y1_col in self.columns:
            y1_index = self.columns.index(y1_col)
            self.lb_y.selection_set(y1_index)
            self.lb_y.see(y1_index)
...
def _parse_cli_args():
    """Parse command-line options for optional cross-app handoff loading.

    Purpose:
        Read launch arguments used by the GL-260 launcher integration.
    Why:
        The standalone script should keep normal behavior while optionally
        accepting a JSON payload path for transferred datasets.
    Inputs:
        None.
    Outputs:
    ...
    Exceptions:
        Invalid CLI usage is handled by argparse with standard error output.
    """
    parser = argparse.ArgumentParser(description="General Plotter")
    parser.add_argument(
        "--handoff-json",
        default="",
