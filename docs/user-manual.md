...
16. Click **Open Plot in New Tab** to send the dashboard plot to the generated plot-tab pipeline.
17. Use the generated plot tab controls for refresh, export, Plot Settings, Data Trace Settings, Plot Elements, and Plot Preview.
...
- The live Reaction Dashboard cycle table always shows both rate columns before export, using the same cached Cycle Analysis values as the Cycle Report.
...
- The optional full Combined Triple Axis page reuses the normal vector PDF export plot at 11 x 8.5 inches and appends it after all Cycle report pages. It can be selected independently of the compact snapshot. This appended page uses complete prepared traces instead of preview-decimated arrays, disables vector-path simplification, and uses at least 1200 DPI for any rasterized plot content.
...
- Repeated cycle report pages/tables are titled with the respective reactor trace label.
- Speciation timeline, pH guidance, planner narrative, key metrics, and solubility sections remain profile-level unless trace-specific data is available.
- The interactive HTML Final Report viewer includes current-profile reactor cycle groups and shows a reactor selector when multiple reactor traces are present.
